...
def _get_train_dataloader(tokenizer, model_config, train_config):
    ...
    train_datasets = [laion_caption, laion_instruct]
...
def get_train_dataloader(tokenizer, model_config, train_config):
    # load recap 558k lazy
    data = load_lmms_lab_recap(
        tokenizer, model_config.instruction_template, model_config.response_template
    )
    seed = 42 + dist.get_rank() if dist.is_initialized() else 42  # hope this helps
    data = data.shuffle(seed=seed, buffer_size=100)
