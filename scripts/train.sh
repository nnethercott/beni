...
export NCCL_SOCKET_IFNAME=enp # apparently gets set by default
...
ADDR=${ADDR:-'127.0.0.1'}
PORT=${PORT:-'29501'}
RUN_NAME_PREFIX=${RUN_NAME_PREFIX:-'vlm'}
RUN_NAME="${RUN_NAME_PREFIX}-$(uuidgen | awk -F'-' '{print $1}')"

AIP_MODEL_DIR=${AIP_MODEL_DIR:-'/mnt/nate/model_checkpoints/test'}
AIP_MODEL_DIR_CLEAN=$(echo "$AIP_MODEL_DIR" | sed 's#gs://#/gcs/#g')
...
torchrun --nproc_per_node="${NUM_GPUS}" --nnodes="${NNODES}" --node_rank="${RANK}" --master_addr="${ADDR}" --master_port="${PORT}" ./src/vlm/hf_main.py \
    ...
    --lora_r 2 --lora_alpha 32 --lora_target_modules "q_proj,k_proj,v_proj,o_proj" --enable_peft true \
    ...
    --fp16 false \
    --dataset_name "recap" --n_samples 10000 \
