    ...
    with gr.Row():
        with gr.Column(scale=2):
            ...
            text_prompt = gr.Textbox(
                lines=2,
                placeholder="Enter a text prompt related to the image",
                label="Text Prompt",
            )
            send_button = gr.Button("Send")
        with gr.Column(scale=3):
            chatbot = gr.Chatbot(
                label="Chat with Beni", type="messages"
            )  # , elem_id = "chatbot")
        ...
        send_button.click(
            fn=add_message,
            inputs=[image_input, text_prompt, chatbot],
            outputs=[chatbot, text_prompt, image_input],
        )
